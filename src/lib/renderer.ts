...
export class SpriteRenderer {
  ...
  async load(bytes: ArrayBuffer) { const url = URL.createObjectURL(new Blob([bytes], { type: "image/webp" })); this.image = new Image(); this.image.src = url; try { await this.image.decode(); } finally { URL.revokeObjectURL(url); } }

...
use tauri::{AppHandle, Manager, WebviewUrl, WebviewWindowBuilder, Emitter, RunEvent};
...
fn builtin_dir(app: &AppHandle) -> Result<PathBuf, String> { app.path().resource_dir().map_err(|e|e.to_string()).map(|p| p.join("pets").join("ikunchick")) }
...
fn custom_record(app:&AppHandle, dir:PathBuf) -> Result<PetRecord,String> { let m=validate_package(&dir)?; Ok(PetRecord{manifest:m,source:"custom".into(),path:dir.to_string_lossy().into_owned()}) }
#[tauri::command] fn list_pets(app:AppHandle)->Result<Vec<PetRecord>,String>{
  let b=builtin_dir(&app)?; let mut pets=vec![PetRecord{manifest:validate_package(&b)?,source:"builtin".into(),path:b.to_string_lossy().into_owned()}];
  let dir=pets_dir(&app)?; if let Ok(entries)=fs::read_dir(dir) { for e in entries.flatten() { if e.path().is_dir() { if let Ok(p)=custom_record(&app,e.path()){pets.push(p)} } } }; Ok(pets)
}
...
#[tauri::command] fn import_pet(app:AppHandle,source_path:String,conflict:String)->Result<PetRecord,String>{
  ...
  copy_dir(&src,&target)?; if conflict=="rename" { fs::write(target.join("pet.json"),serde_json::to_vec_pretty(&m).map_err(|e|e.to_string())?).map_err(|e|e.to_string())? }; custom_record(&app,target)
}

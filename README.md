...
“输入动作监听”默认开启，也可以在设置页关闭并持久化。每只宠物可以单独选择输入时播放的标准动画，默认是 `running`；选择结果按宠物 ID 保存，切换回来时自动恢复。开启时，应用会监听系统级键盘活动，最后一次按键后的 800ms 恢复 `idle`。监听器只判断是否发生按键，不读取、保存或传输具体字符。macOS 开发运行时，需要在“系统设置 → 隐私与安全性 → 辅助功能”中允许启动应用的终端；安装版则允许 Pet Desk。
